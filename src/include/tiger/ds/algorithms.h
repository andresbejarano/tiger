...
{
    //
    // Checks if all face vertices have the ATTRIB_COPLANAR attribute set with 
    // value true.
    // @param const std::shared_ptr<dcel::Face> face The pointer to a face.
    // @return bool Indicates if all face vertices have the ATTRIB_COPLANAR 
    // attribute set to true.
    //
    bool areFaceVerticesCoplanar(const std::shared_ptr<dcel::Face> face);

    //
    // Calculates the parameters for the point where a face and a ray 
    // intersect.
    // @param const std::shared_ptr<dcel::Face> face The pointer to a face in a
    // DCEL.
    ...
    //
    // Finds the intersection points of a DCEL face F1 with another DCEL face 
    // F2. Note that this approach does not calculate the intersection points 
    // of face F2 with face F1 since they are not necessarily the same.
    ...
    // where the intersection points will be stored.
    // @param double threshold The threshold for values close to zero.
    // @return double Indicates if there is at least one intersection point 
    // between the faces.
    //
    bool intersectCoplanarFaces(
    ...
    //
    // @param const std::vector<dcel::DCEL> & blocks The reference to the 
    // vector with the geometry of the blocks.
    // @param const std::vector<dcel::DCEL> & interfaces The reference to the 
    // vector with the geometry of the interface polygons.
    //
