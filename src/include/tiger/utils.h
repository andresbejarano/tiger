...
namespace utils
{
    ...
    // 
    // Rotates vector V around an axis vector K and angle of rotation a. This 
    // method is also named as Rodrigues' rotation formula: 
    // 
    // (V * cos(a)) + ((K x N) * sin(a)) + (K * (K . V) * (1 - cos(a)))
    // 
    // Its description is in:
    // 
    // https:// en.wikipedia.org/wiki/Rodrigues%27_rotation_formula
    // 
    // @param const Eigen::Vector3d & V The reference to the vector to be 
    // rotated.
    // @param const Eigen::Vector3d & K The reference to the rotation axis 
    // vector.
    ...
    // @param const std::list<Eigen::Vector3d> & points The reference to a list
    // with the points.
    ...
    // @param const std::vector<Eigen::Vector3d> & points The reference to a 
    ...
    // 
    // Finds a path between the points in a graph represented by a directed 
    // adjacency matrix.
    // @param Eigen::MatrixXd & adj The reference to the adjacency matrix.
    // @param std::vector<bool> & visited The reference to the vector 
    // indicating which points have been visited.
    // @param size_t i The start point index of the path.
    // @param std::vector<size_t> & path The reference to the vector with the 
    // sequence of indices that form the path.
    ...
    // @param std::vector<Eigen::Vector3d> & V The reference to the vector with
    // the 3D vectors.
    ...
    // with the points.
    // @param std::vector<Eigen::Vector3d> & unique The reference to the vector
    // where unique points
    // will be stored.
    // @param double threshold The threshold for values close to zero.
    ...
    // @param const Eigen::Vector3d & v0 The reference to a vertex of the 
    // triangle.
    // @param const Eigen::Vector3d & v1 The reference to a vertex of the 
    // triangle.
    // @param const Eigen::Vector3d & v2 The reference to a vertex of the 
    // triangle.
    // @param double threshold The threshold for values close to zero.
    ...
    // @param std::vector<double> & values The vector with the values for the 
    // function.
    ...
    // @param bool newline Indicates whether to write a new line character 
    // after the vector or not.
    // 
