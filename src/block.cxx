    ...
    double threshold) const
{
    ...
    for(size_t block1FaceIdx = 0; block1FaceIdx < nBlock1Faces; block1FaceIdx += 1)
    {
        ...
        utils::intersectCoplanarFaces(m_vf, block1FaceIdx, block->Geometry(), otherFaceIdx, V1, threshold);
        utils::intersectCoplanarFaces(block->Geometry(), otherFaceIdx, m_vf, block1FaceIdx, V2, threshold);
        ...
        if (!utils::sortPointsCCW(uniquePoints, plane, ccwPoints, threshold))
        {
            return;
        }
