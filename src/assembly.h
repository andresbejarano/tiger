...
/*
The class representing an assembly of interlocking blocks. Tile tessellations 
and blocks are a 1-to-1 match. That is, the index of the tile in the respective
data structure is the same index of the respective block in the assembly.
*/
class Assembly 
{
...
public:
    ...
    */
    ...
    /*
    Constructor of the class.
    @param const std::vector<VF> & blocks The reference to the vector with the 
    ...
    */
    ...
    tessellation with the information of the faces associated to the blocks.
    @param std::string & topFunction The top adaptive function.
    @param const std::string & bottomFunction The bottom adaptive function.
    @param std::vector<VF> & clipped The vector to store the pointers to the 
    clipped blocks.
    ...
    */
    ...
    /*
    Indicates if there is at least one block that has been modified. A modified
    block will have the ATTRIB_MODIFIED dynamic attribute and its value is true.
    ...
    */
    ...
    @param std::shared_ptr<Tessellation> tessellation The pointer to the 
    tessellation.
    @param std::vector<std::shared_ptr<VF>> & interfaces The reference to the 
    vector for storing the pointers to the geometries of the interface polygons.
    @param double threshold The threshold for values close to zero.
    ...
    */
    ...
    @param std::shared_ptr<Tessellation> tessellation The pointer to the 
    tessellation.
    @param std::vector<std::shared_ptr<VF>> & overlapping The reference to the 
    vector to store the pointers to the geometries of the overlapping.
    @param double threshold The threshold for values close to zero.
    ...
    */
    ...
    @param std::vector<Eigen::Vector3d> & C The reference to the vector for 
    ...
    */
    ...
    /*
    Returns the number of triangles required to represent the blocks of the 
    assembly.
    @return size_t The number of triangles required to represent the blocks in 
    the assembly.
    */
    ...
    */
    ...
    /*
    Returns the reference to the vector with the pointers to the blocks of the 
    assembly.
    @return std::vector<std::shared_ptr<Block>> & The reference to the vector 
    with the pointers to the blocks of the assembly.
    */
    ...
    */
    ...
    /*
    Returns a vector with the loads of the blocks. The are six values in per 
    block vector: three force loads along the X, Y and Z axes, and three 
    momentum loads along the X, Y and Z axes. All but the Y force load are set 
    to zero. Y force load is the volume of the block times the density value 
    times -9.8.
    @param double density The density of the blocks.
    @return std::vector<std::vector<double>> The vector with the loads that 
    apply to the blocks.
    */
    ...
    @param const std::shared_ptr<Tessellation> tessellation The pointer to the 
    tessellation that contains the face associated to the block.
    @param size_t index The index of the block.
    @param std::vector<double> & lenghts The vector to store the central length
    values from the neighboring blocks of the given block.
    ...
    */
    ...
    /*
    Sets the blocks. This method clears any previous content and copies the 
    pointers given in the vector, no new objects are generated.
    @param std::vector<std::shared_ptr<block>> & blocks The reference to the 
    vector with the pointers to the blocks.
    */
    //void Set(std::vector<std::shared_ptr<block>> & blocks);

    /*
    Sets the blocks. This method clears any previous content. Dynamic 
    attributes from the VF objects are not copied.
    @param std::vector<std::shared_ptr<block>> & blocks The reference to the 
    vector with the pointers to the blocks.
    */
    ...
    /*
    Sets the indices of the blocks. Each interface keeps its index using the 
    ...
    */
    ...
    /*
    Enables or disables the blocks according to their location. A block is 
    enabled if its associated face in the tessellation has all of its neighbors
    (aka not at the boundary); otherwise, it is disabled (since it is at the 
    boundary).
    @param std::shared_ptr<Tessellation> tessellation The pointer to the 
    tessellation.
    */
