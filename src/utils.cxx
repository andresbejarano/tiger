...
bool utils::areFaceVerticesCoplanar(const std::shared_ptr<dcel::Face> face)
{
...
bool utils::faceRayIntersection(
...
bool utils::getIntersectionPointsFromPlanes(
...
bool utils::intersectCoplanarFaces(
    const VF & vf1, 
    size_t face1, 
    const VF & vf2, 
    size_t face2, 
...
/*double utils::Min(std::vector<double> & values)
{
    std::vector<double>::iterator result = std::min_element(values.begin(), values.end());
    return *result;
}*/

bool utils::planeLineSegmentIntersection(
    const toolkit::Plane & plane,
    ...
    double threshold)
{
    ...
    return planeRayIntersection(plane, ray, t, threshold);
}

bool utils::planeRayIntersection(
    const toolkit::Plane & plane,
    const toolkit::Ray & ray,
    double & t,
    double threshold)
...
bool utils::sortPointsCCW(
    ...
    double threshold)
{
    return sortPointsCCW(points, plane.Normal(), plane.Point(), sorted, threshold);
}

bool utils::threePlanesIntersection(
    ...
    Eigen::Vector3d & P)
{
    ...
    double D = det(Aa, Ab, Ac, Ba, Bb, Bc, Ca, Cb, Cc);
    ...
    double D1 = det(Ad, Ab, Ac, Bd, Bb, Bc, Cd, Cb, Cc);
    double D2 = det(Aa, Ad, Ac, Ba, Bd, Bc, Ca, Cd, Cc);
    double D3 = det(Aa, Ab, Ad, Ba, Bb, Bd, Ca, Cb, Cd);
...
void utils::write(const std::vector<dcel::DCEL> & V)
{
    for (auto it = V.begin(); it != V.end(); ++it)
    {
        (*it).Write();
