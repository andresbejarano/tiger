...
length = 1
Seedtop = planar.regularpolygon(6, length)

# Set the angle and height values
angles = [utils.HALF_PI - math.atan(1 / (2 * (2 ** 0.5)))] * 6
angles[1] *= -1
angles[3] *= -1
angles[5] *= -1
#height = length * (2 / 3) ** 0.5
height = 0.37
...
# Calculate the points resultant from the positive evolution step
toppoints = evolve(Seedtop, angles, height, allpoints, alledges, allfaces)

Seedbottom = Seedtop.copy().flip()
for i in range(len(angles)):
    angles[i] *= -1

# Calculate the points resultant from the negative evolution step
bottompoints = evolve(Seedbottom, angles, height, allpoints, alledges, allfaces)
