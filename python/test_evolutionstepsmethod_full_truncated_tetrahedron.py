...
toppoints2 = evolve(Seed2top, [-1 * abs(a) for a in angles], height / 2, allpoints, alledges, allfaces)

Seed1bottom = Seed1top.copy().flip()
for i in range(len(angles)):
    angles[i] *= -1

bottompoints1 = evolve(Seed1bottom, angles, height / 2, allpoints, alledges, allfaces)

# Use the points to define the seed polygon for the second negative evolution
# step
Seed2bottom = VF()
for p in bottompoints1:
    Seed2bottom.addvertexfrompvector(p)
Seed2bottom.addface([i for i in range(len(bottompoints1))])

bottompoints2 = evolve(Seed2bottom, [-1 * abs(a) for a in angles], height / 2, allpoints, alledges, allfaces)
